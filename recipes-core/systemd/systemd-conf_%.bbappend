...
do_install:prepend:dh-dhsom() {
    if [ "${@d.getVar('LAYERSERIES_CORENAMES') in ['scarthgap']}" = "True" ]; then
        UNPACKWORKDIR="${WORKDIR}"
    else
        UNPACKWORKDIR="${UNPACKDIR}"
    fi
}

do_install:append:dh-dhsom() {
	install -D -m0644 ${UNPACKWORKDIR}/80-rtc-dhsom.rules \
			  ${D}${sysconfdir}/udev/rules.d/80-rtc-dhsom.rules
	install -D -m0644 ${UNPACKWORKDIR}/80-ethsom0.link \
			  ${D}${systemd_unitdir}/network/80-ethsom0.link
	install -D -m0644 ${UNPACKWORKDIR}/80-ethsom0.network \
			  ${D}${systemd_unitdir}/network/80-ethsom0.network
	if ${@bb.utils.contains('IMAGE_FEATURES', 'post-install-logging', 'true', 'false', d )} ; then
		install -D -m0644 ${UNPACKWORKDIR}/80-etnaviv-devcoredump.rules \
			  ${D}${sysconfdir}/udev/rules.d/80-etnaviv-devcoredump.rules
		install -D -m0755 ${UNPACKWORKDIR}/etnaviv-devcoredump \
			  ${D}${bindir}/etnaviv-devcoredump
	fi
}

do_install:append:dh-stm32mp1-dhsom() {
	install -D -m0644 ${UNPACKWORKDIR}/80-ethsom1.link \
			  ${D}${systemd_unitdir}/network/80-ethsom1.link
	install -D -m0644 ${UNPACKWORKDIR}/80-ethsom1.network \
			  ${D}${systemd_unitdir}/network/80-ethsom1.network
	install -D -m0644 ${UNPACKWORKDIR}/80-wlansom0.link \
			  ${D}${systemd_unitdir}/network/80-wlansom0.link
	install -D -m0644 ${UNPACKWORKDIR}/80-wlansom0.network \
...
do_install:append:dh-stm32mp13-dhcor-dhsbc() {
	install -D -m0644 ${UNPACKWORKDIR}/80-phy-dhsbc.rules \
			  ${D}${sysconfdir}/udev/rules.d/80-phy-dhsbc.rules
	install -D -m0644 ${UNPACKWORKDIR}/logind-powerkey.conf \
			  ${D}${systemd_unitdir}/logind.conf.d/01-${PN}.conf
}

do_install:append:dh-stm32mp1-dhcom-drc02() {
	install -D -m0644 ${UNPACKWORKDIR}/80-ethusb0.link \
			  ${D}${systemd_unitdir}/network/80-ethusb0.link
	install -D -m0644 ${UNPACKWORKDIR}/80-ethusb0.network \
